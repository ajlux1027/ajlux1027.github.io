...
function ticTac(userSelection) {
    ...
    let input = 0;
    if(turn % 2 == 0) {	
    //Checks if turn count is even or odd for user
        character = 'X';
        ...
        if(input >= 0 && input <= 8) {
            ...
            // decides if spot is already taken
                 turn--;
            else {
                boxes[input].innerText = character;		
                // If spot is not taken, X/O placed in position
        ...
        if(turn % 2 == 1) {
            ...
            let temporary = -1;
            character = 'O';		
            //O placed on odd turns
            input = -1;
        	input = ticTacBrain('O');        
            //Possible wins checked first
            console.log("Robot input is " + input);
       		temporary = ticTacBrain('X');    
            //Defensive moves checked if no wins present
            console.log("Robot temp is " + temporary);
       		
       		if(input == -1 && temporary > -1) input = temporary;
           
            if(input == -1) input = ticTacBackup(); 
            //Advacned strategy accessed if no drasic moves exist
        ...
        winState();			
        //Check for possible win after each move
...
function winState() {	
//Compares current inputs. All win conditions independently checked
    
    if((boxes[0].innerText == boxes[3].innerText) && (boxes[3].innerText == boxes[6].innerText))  		      
    //Win on Left Column
    ...
    if((boxes[1].innerText == boxes[4].innerText) && (boxes[4].innerText == boxes[7].innerText)) 		  
    ...
    if((boxes[2].innerText == boxes[5].innerText) && (boxes[5].innerText == boxes[8].innerText)) 	      
    //Win on Right Column
    ...
    if((boxes[3].innerText == boxes[4].innerText) && (boxes[4].innerText == boxes[5].innerText))         
    //Win on Middle Row
    ...
    if((boxes[6].innerText == boxes[7].innerText) && (boxes[7].innerText == boxes[8].innerText)) 	      
    //Win on Bottom row
    ...
    if(turn == (9)) {											
	    //Tie game declared when all spaces filled without full row/column/diagonal
...
function declareWinner(val) {		                              
	//Displays the Winner and the final board
    switch(val) 
    {
        case 'X': 
        	console.log("X wins!");
